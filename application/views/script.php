...
<script>
  $(function () {
    var tiempo = 2000 //Fetch data ever x milliseconds

function obtenerDatos() {

    console.log((new Date()).getTime());
    $.ajax({
        url: 'index.php/pulso/obtenerDatos',
        dataType: "json",
        method: 'POST',
        success: function (data) {
           console.log(data);
           dibujarPieBpm(data.bpm);
           dibujarPieSaturacion(data.saturacion);
        },
    ...
    setTimeout(obtenerDatos, tiempo)
}
obtenerDatos();
function dibujarPieBpm(dataPieBpm){
    
    $.plot('#donut-bpm', dataPieBpm, {
      series: {
        pie: {
          show       : true,
          radius     : 1,
...
function dibujarPieSaturacion(dataPieSaturacion){
    
    $.plot('#donut-saturacion', dataPieSaturacion, {
      series: {
        pie: {
          show       : true,
          radius     : 1,
          ...
          label      : {
            show     : true,
            radius   : 2 / 3,
            formatter: labelFormatterSaturacion,
            threshold: 0.1
          }
  ...
  function labelFormatterSaturacion(label, series){
    return '<div style="font-size:13px; text-align:center; padding:2px; color: #fff; font-weight: 600;">'
      + label
      + '<br>'
      + Math.round(series.percent) + '%</div>'
